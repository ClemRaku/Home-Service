...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const loadProfile = async () => {
    ...
    try {
      ...
      // Re-create icons in case any were replaced
      if (typeof lucide !== 'undefined') lucide.createIcons();
    } catch (error) {
  ...
  if (cameraButton && avatarInput) {
    cameraButton.addEventListener('click', () => avatarInput.click());
  }
  ...
  if (avatarInput && profileAvatar) {
    avatarInput.addEventListener('change', (event) => {
      ...
      reader.onload = (loadEvent) => { profileAvatar.src = loadEvent.target.result; };
  ...
  const setEditModeUI = (editing) => {
    ...
    if (editing) {
      ...
      editProfileBtn.innerHTML = '<span>Save Profile</span>';
